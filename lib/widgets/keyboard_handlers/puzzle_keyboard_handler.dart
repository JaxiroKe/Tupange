...
class _PuzzleKeyboardHandlerState extends State<PuzzleKeyboardHandler> {
  ...
  void _onStart(bool hasStarted) {
    ...
    context.read<GameBloc>().add(CountdownReset(
  ...
  void _handleKeyEvent(RawKeyEvent event) {
    if (event is RawKeyDownEvent) {
      ...
      final planetPuzzleState = context.read<GameBloc>().state;

      final isAutoSolving =
          context.read<PuzzleHelperCubit>().state.isAutoSolving;

      final isReady = puzzleInitState is PuzzleInitReady;
      final hasStarted = planetPuzzleState.status == PlayingStatus.started;
      final isLoading = planetPuzzleState.status == PlayingStatus.loading;
